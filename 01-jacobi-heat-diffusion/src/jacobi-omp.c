...
void jacobi_update(double *grid, double *grid_new) {
    #pragma omp parallel for num_threads(16) // parallel region
    for (int i = HY; i < GY - HY; i++) { // - HY così non tocco mai i ghost borders
        for (int j = HX; j < GX - HX - K; j++) { // non aggiorno la piastra nelle K-colonne
            int index = i * GX + j; // corrisponde alle coordinate [i,j] nelle matrix
...
void jacobi_update(double *grid, double *grid_new, int nt) {
    #pragma omp parallel for collapse(2) num_threads(nt) // collassa il numero di loop interni in uno unico esterno per poter essere parallelizzato
    for (int i = HY; i < GY - HY; i++) {
        for (int j = HX; j < GX - HX - K; j++) {
            int index = i * GX + j;
            grid_new[index] = (
                grid[(i - 1) * GX + j] +
                grid[(i + 1) * GX + j] +
...
int main() {
    ...
    for(int num_threads = 1; num_threads <= max_threads; num_threads++)
    {
        ...
        #if DUMP == 1
            sprintf(filename, "video-omp/grid-%07d",0);
            dump(grid, filename);
        #endif
        start_time = omp_get_wtime();
        for(iter = 1; iter <= MAXITER; iter++){
            //jacobi_update(...)
            jacobi_update(grid, grid_new,num_threads);
            //dumpiter(...)
            #if DUMP == 1
            if(iter%DUMPSTEP == 0){
                sprintf(filename, "video-omp/grid-%07d",iter);
        ...
        //statistic info
        printf("[statistics] num_threads: %d lattice %dx%d %d iter dT: %.2f msec dT/iter: %.2f usec GFLOPs: %.2f\n",
                num_threads,GLX, GLY, MAXITER, end_time, (end_time*1000.0)/(double)MAXITER, 
